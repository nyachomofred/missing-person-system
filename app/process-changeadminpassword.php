...
if (isset($_POST['btnChangePass'])) {
	...
	$sql="SELECT `Password` FROM system_admin";
	...
	if (strlen($newpass)<8) {
		...
		<script type="text/javascript">
			...
			window.location.assign('../views/editadminprofile.php');
		</script>
		<?php
		...
		$sqlUpdate="UPDATE `missingpersons`.`system_admin` SET `Password` = '".$encyptedpass."' WHERE `system_admin`.`UserID` = 'franklinokech@gmail.com' ";
		//run query
