...
if (isset($_POST['btnChangePass'])) {
	...
	$sql="SELECT `Password` FROM searcher WHERE `Username`='".$_SESSION['Searcher']."'";
	...
	if (strlen($newpass)<8) {
		...
		<script type="text/javascript">
			...
			window.location.assign('../views/searcher/editadminprofile.php');
		</script>
		<?php
		...
		$sqlUpdate="UPDATE `missingpersons`.`searcher` SET `Password` = '".$encyptedpass."' WHERE `searcher`.`Username` = '".$_SESSION['Searcher']."' ";
		//run query
