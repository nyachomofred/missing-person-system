...
if($num_row==1){
    ...
    $sqlgetadminid="SELECT `National_ID` FROM user WHERE `Username`='".$_SESSION['Authorised']."' ";
    $sqlrun=mysqli_query($conn,$sqlgetadminid);
    while ($rowid=mysqli_fetch_assoc($sqlrun)) {
        $nationalid=$rowid['National_ID'];
    }

	if($nationalid==$reporterid){
		    if($img==""){
        ?>
    <div class="container">
    ...
    <h3><?php echo $fullname;?></h3>
		...
		?>
  <form action="../../app/process-authorisedreunite.php" method="POST">
    <input type="hidden" name="txtid" id="txtid" value="<?php echo $identity;?>">
    <input type="submit" name="btnreportfound"  id="btnreportfound" class="btn btn-success" value="RE-UNITE">
    
</form>
<?php
		
		}//end of if
		elseif($nationalid!=$reporterid){
			if($img==""){
        ?>
    <div class="container">
    <img src="../../storage/img/default.jpg" alt="..." class="img-responsive">
