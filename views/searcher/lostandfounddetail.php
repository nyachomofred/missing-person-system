...
if($num_row==1){
    ...
    $sqlgetadminid="SELECT `ID_No` FROM searcher WHERE `Username`='".$_SESSION['Searcher']."' ";
    $sqlrun=mysqli_query($conn,$sqlgetadminid);
    while ($rowid=mysqli_fetch_assoc($sqlrun)) {
        $nationalid=$rowid['ID_No'];
    }
    ...
    <h3><?php echo $fullname;?></h3>
		...
		?>
  <form action="../../app/process-searcherreunite.php" method="POST">
